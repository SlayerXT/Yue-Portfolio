...
const Hero = ({ onUnlock, onSkip }: HeroProps) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const clarity = (precision + logic) / 200;
  ...
  // Handle precision knob hover - SPEED UP: 1 second instead of 2
  const handlePrecisionHoverStart = useCallback(() => {
    ...
    precisionInterval.current = setInterval(() => {
      setPrecision(prev => {
        ...
        return prev + 4; // 4% per 40ms = 100% in 1 second (faster!)
      });
    }, 40);
  }, [isPrecisionLocked]);
  ...
  // Handle logic knob hover - SPEED UP: 1 second instead of 2
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4">
        ...
        <p className="text-lg md:text-xl font-mono text-osc-green tracking-[0.3em] uppercase mb-12">
        ...
        <button
          onClick={onSkip}
          className="flex items-center gap-2 px-4 py-2 text-sm font-mono text-gray-500 hover:text-osc-green border border-gray-800 hover:border-osc-green/50 rounded transition-all duration-300"
        >
